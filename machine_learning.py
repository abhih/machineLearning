...
FORMAT = "[%(filename)s:%(lineno)s:%(levelname)-8s-%(funcName)20s()] %(message)s"
...
def tokenize(str):
    ...
    return [word_tokenize(" ".join(re.findall(r'\w+', t )).lower())#, flags=re.UNICODE
...
def preprocess_pipeline(str, lang="english", stemmer_type="PorterStemmer", return_as_str=False,
    ...
    for sentence in sentences:
        if do_remove_stopwords:
            ...
            words = sentence
        logger.debug('words_sentence:%s',words)
        #print("words1",words)
        words = (stemming(words, stemmer_type))
        #print("words stemming",words)
        words_str=[]
        if return_as_str:
            words_str=[word.decode("utf-8") for word in words ]
            #print("words_str",words_str)
            l.append(" ".join(words_str))
        else:
            l.append(words)
    if return_as_str:
        return " ".join(l)
...
print ("\nPorter:\n", preprocess_pipeline(test_sentence, "english", "PorterStemmer", True, False, False))
print ("\nLancaster:\n", preprocess_pipeline(test_sentence, "english", "LancasterStemmer", True, False, False))
print ("\nWordNet:\n", preprocess_pipeline(test_sentence, "english", "WordNetLemmatizer", True, False, False))
print ("\nStopword Tokenized Lancaster:\n", preprocess_pipeline(test_sentence, "english", "LancasterStemmer", False, True, False))
#print ("\nOnly cleaning (HTML+Text):\n", preprocess_pipeline(test_sentence, "english", False, True, False, False))